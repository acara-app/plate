...
function CustomTooltip({ active, payload, glucoseUnit }: TooltipProps) {
    ...
    if (active && payload && payload.length) {
        ...
        let status = t('health_entries.glucose_chart.status.normal');
        let statusColor = 'text-green-600';
        let contextNote = '';

        if (value < thresholds.low) {
            status = t('health_entries.glucose_chart.status.low');
            statusColor = 'text-orange-600';
        } else if (value > thresholds.high) {
            status = t('health_entries.glucose_chart.status.high');
            statusColor = 'text-red-600';
        } else if (value > thresholds.normalMax) {
            status = t('health_entries.glucose_chart.status.elevated');
        ...
        if (data.isFasting) {
            contextNote = t('health_entries.glucose_chart.targets.fasting', {
                target: thresholdConfig.normal,
                unit: glucoseUnit,
            });
        } else {
            contextNote = t('health_entries.glucose_chart.targets.post_meal', {
                target: thresholdConfig.normal,
                unit: glucoseUnit,
            });
        }
...
export default function GlucoseChart({ readings, glucoseUnit }: Props) {
    ...
    if (readings.length === 0) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle>
                        {t('health_entries.glucose_chart.title')}
                    </CardTitle>
                    <CardDescription>
                        {t('health_entries.glucose_chart.description')}
                    </CardDescription>
                </CardHeader>
                <CardContent>
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>{t('health_entries.glucose_chart.title')}</CardTitle>
                <CardDescription>
                    {t('health_entries.glucose_chart.description_with_unit', {
                        unit: glucoseUnit,
                    })}
                </CardDescription>
            </CardHeader>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm">
                            ...
                            <>
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-muted-foreground">
                                        {t(
                                            'health_entries.glucose_chart.legend.fasting_normal',
                        ...
                        {hasPostMeal && (
                            <>
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-muted-foreground">
                                        {t(
                                            'health_entries.glucose_chart.legend.post_meal_normal',
                                            {
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-muted-foreground">
                                {t('health_entries.glucose_chart.legend.low', {
                        ...
                        {isSparse && hasMovingAvg && (
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-muted-foreground">
                                    {t(
                                        'health_entries.glucose_chart.legend.trend',
                                    )}
                                </span>
                            </div>
                        )}
                    ...
                    {hasMixedTypes && (
                        <div
                            ...
                            dangerouslySetInnerHTML={{
                                __html: t(
                                    'health_entries.glucose_chart.context_banner',
                                ),
                            }}
                        />
                    )}
                        ...
                        >
                            ...
                            {hasPostMeal && (
                                <ReferenceLine
                                    ...
                                    label={{
                                        value: t(
                                            'health_entries.glucose_chart.reference_labels.post_meal',
                                        ),
                                        position: 'right',
                                        fill: '#eab308',
                                        fontSize: 10,
                            ...
                            {hasFasting && (
                                <ReferenceLine
                                    ...
                                    label={{
                                        value: t(
                                            'health_entries.glucose_chart.reference_labels.fasting',
                                        ),
                                        position: 'right',
                                        fill: '#10b981',
                                        fontSize: 10,
                            ...
                            <ReferenceLine
                                ...
                                label={{
                                    value: t(
                                        'health_entries.glucose_chart.reference_labels.low',
                                    ),
                                    position: 'right',
                                    fill: '#f97316',
                                    fontSize: 10,
                    ...
                    <div className="border-t pt-4">
                        <p className="mb-2 text-sm font-medium">
                            {t(
                                'health_entries.glucose_chart.reading_types_title',
                            )}
                        </p>
