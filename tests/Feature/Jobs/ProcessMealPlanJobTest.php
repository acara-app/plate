...
it('generates and stores meal plan incrementally', function (): void {
    ...
    WorkflowStub::mock(CreateMealPlanActivity::class, function ($context, $activityUser, $totalDays) use ($user) {
        return $user->mealPlans()->create([
            ...
            'metadata' => ['status' => MealPlanGenerationStatus::Generating->value, 'days_completed' => 0],
        ]);
    });
    ...
    // Generate all 7 days by setting initialDays to 7
    $job = new ProcessMealPlanJob($user, AiModel::Gemini25Flash, totalDays: 7, initialDays: 7);
    $job->handle();
...
it('generates only initial day by default for on-demand generation', function (): void {
    WorkflowStub::fake();
    ...
    expect($mealPlan)
        ...
        ->meals->toHaveCount(1); // Only 1 day generated initially

    // Verify only 1 day's activities were dispatched
    WorkflowStub::assertDispatched(CreateMealPlanActivity::class, 1);
    WorkflowStub::assertDispatched(GenerateDayMealsActivity::class, 1);
    WorkflowStub::assertDispatched(StoreDayMealsActivity::class, 1);
});
