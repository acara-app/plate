...
beforeEach(function (): void {
    config()->set('snap', ['enabled' => true, 'model_approved' => true,
        ...
        'free_daily_scans' => 1, 'monthly_scans' => 100]);
});

it('shares one free daily scan across entry points and restores it next UTC day', function (): void {
    ...
    $this->travel(1)->days();
    expect($access->analyze(new PhotoAnalysisContext($user, null, 'third', 'chat', 'other'), fn () => $result))->toBe($result);
});
...
it('grants exactly 100 premium scans from paid invoice periods and preserves cancellation grace', function (): void {
    ...
    App\Models\SubscriptionProduct::factory()->create(['name' => 'Snap Pro', 'stripe_price_id' => 'price_snap']);
...
it('hides legacy offers while preserving their prices and keeps photo costs outside the monthly AI budget', function (): void {
    ...
    $snap = App\Models\SubscriptionProduct::factory()->create(['name' => 'Snap Pro', 'price' => 9, 'stripe_price_id' => 'price_snap']);
    ...
    expect($offers->available($legacy))->toBeFalse()
        ...
        ->and($offers->present($free)->features)->toContain('1 standard photo scan per day')
...
it('keeps paid photo scans available during a sales rollback when other AI is exhausted', function (): void {
    ...
    App\Models\SubscriptionProduct::factory()->create(['name' => 'Snap Pro', 'stripe_price_id' => 'price_rollback']);
